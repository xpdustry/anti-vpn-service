...
public class CronExpression {
    enum CronFieldType {
        ...
        MONTH(1, 12,
                Seq.with("JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC")) {
            @Override
            int getValue(ZonedDateTime dateTime) {
        ...
        DAY_OF_WEEK(1, 7, Seq.with("MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN")) {
            @Override
            int getValue(ZonedDateTime dateTime) {
        ...
        final int from, to;
        final Seq<String> names;

        CronFieldType(int from, int to, Seq<String> names) {
            this.from = from;
            this.to = to;
            this.names = names;
        }
    ...
    /**
     * Will search for the next time within the next 4 years. <br>
     * If there is no time matching, an {@code InvalidArgumentException} 
     * will be thrown (it is very likely that the cron expression 
     * is invalid, like the February 30th).
     */
    ...
    /**
     * Will search for the next time within the next {@code durationInMillis} millisecond. <br>
     * Be aware that the duration is specified in millis, but in fact the limit is checked 
    ...
    abstract static class BasicField {
        ...
        final Seq<FieldPart> parts = new Seq<>();
    ...
    static class DayOfWeekField extends BasicField {
        ...
        @Override
        protected void validatePart(FieldPart part) {
            if (part.modifier != null && !"L".equals(part.modifier) && !"?".equals(part.modifier)) {
                throw new IllegalArgumentException(String.format("Invalid modifier [%s]", part.modifier));
            } else if (part.incrementModifier != null && !"/".equals(part.incrementModifier) && !"#".equals(part.incrementModifier)) {
                throw new IllegalArgumentException(String.format("Invalid increment modifier [%s]", part.incrementModifier));
            }
        }
    }
    ...
    static class DayOfMonthField extends BasicField {
        ...
        @Override
        protected void validatePart(FieldPart part) {
            if (part.modifier != null && !"L".equals(part.modifier) && !"W".equals(part.modifier) && !"?".equals(part.modifier)) {
                throw new IllegalArgumentException(String.format("Invalid modifier [%s]", part.modifier));
            } else if (part.incrementModifier != null && !"/".equals(part.incrementModifier)) {
                throw new IllegalArgumentException(String.format("Invalid increment modifier [%s]", part.incrementModifier));
            }
